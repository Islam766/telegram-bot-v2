...
def callback_inline(call):
    ...
    if call.message:
        try:
            ...
            # Игра-крокодил
            if call.data == "word": 
                if str(crocodile.__annotations__["who_start"]) in str(call.from_user.id):
                    word_txt = crocodile.__annotations__['word_txt']
                    bot.answer_callback_query(callback_query_id=call.id, text=f'Ваше слово: {word_txt}')
                else:
            ...
            if call.data.startswith("new_word"):
                ...
                if str(id_user) in str(call.from_user.id):
                    ...
                    bot.answer_callback_query(call.id, text=f"Ваше слово: {word_txt}")
                else:
                    bot.answer_callback_query(call.id, text="Вы не ведущий!")
            ...
            ######### Обработка ЧЕРНОГО 'ролла' при условии, что человек не нажимал кнопку
            if call.data.startswith("black_roll") and call.from_user.first_name not in text_info_user["names"]:
                text_info_user["names"].append(call.from_user.first_name)
                text_info_user["colors"].append("⚫")
                edit_message_callback(call)
            #########

            ######### Обработка КРАСНОГО 'ролла' при условии, что человек не нажимал кнопку
            if call.data.startswith("red_roll") and call.from_user.first_name not in text_info_user["names"]:
                text_info_user["names"].append(call.from_user.first_name)
                text_info_user["colors"].append("🔴")
                edit_message_callback(call)
            #########

            ######### Обработка ЗЕЛЕНОГО 'ролла' при условии, что человек не нажимал кнопку
            if call.data.startswith("green_roll") and call.from_user.first_name not in text_info_user["names"]:
                text_info_user["names"].append(call.from_user.first_name)
                text_info_user["colors"].append("🟢")
                edit_message_callback(call)
            ...
            if call.data == "roll_fast":
                ...
                if roll_winner is None:
                    bot.edit_message_text(chat_id = call.message.chat.id, message_id = call.message.message_id,
                                        ...
                                        parse_mode = "Markdown", reply_markup = keyboard_delete_last)
        ...
        ########################################
            ...
            if call.data == "delete_full":
                ...
                keyboard_delete = types.InlineKeyboardButton (text = "❌", callback_data = "delete_news")
                keyboard_one =  types.InlineKeyboardButton(text = "1⃣", callback_data = "b_zero")
                keyboard_two =  types.InlineKeyboardButton (text = "2⃣", callback_data = "b_one")
                keyboard_tree = types.InlineKeyboardButton (text = "3⃣", callback_data = "b_two")
                keyboard_next = types.InlineKeyboardButton (text = "🔜", callback_data = "next")
                ...
                if number == 3:
                    ...
                    for line in [0,1,2]: # Превращаем обычный текст в ссыль
                        full.append(f"[{titles_3[line]}]({full_links_3[line]})")
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text = "\n\n".join(full), reply_markup = keyboard, parse_mode = "Markdown")
                elif number == 2:
                    keyboard.add(keyboard_one, keyboard_two, keyboard_delete, keyboard_next)

                    for line in [0,1]: # Превращаем обычный текст в ссыль
                        full.append(f"[{titles_3[line]}]({full_links_3[line]})") #
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,text = "\n\n".join(full), reply_markup = keyboard, parse_mode = "Markdown")

                elif number == 0:
                    full = ("*😫😫 Новоcти кончились 😫😫*")
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,text = full, reply_markup = keyboard_delete_, parse_mode = "Markdown")
