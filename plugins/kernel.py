...
def kernel(m):
    ...
    for line in versions:
        versions_list.append(line.get_text())
        # Получаем список с версиями ядра

    for line in names:
        names_list.append(line.get_text())
        # Получаем смешанный список с названием и ссылями

    for line in names_list:
        if ":" in line:
            sort_names_list.append(line)
            # Убираем все ненужное, отделяем ссылки
    ...
    for line in dates:
        sort_date_list.append(line.get_text())
        # Получаем даты, без всякой херни
    full_info = list(zip(sort_names_list,
                         versions_list,
                         sort_date_list,
                         sort_links_list))
    ##############################################################
