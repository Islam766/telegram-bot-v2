...
def search(m):
    ...
    try:
        ...
        driver.get(f"https://duckduckgo.com/?q={info_user}+russian&ia=web")
        SEARCH_DROP = slice(5)

        finds = driver.find_elements_by_xpath("//a[@class='result__a']"
                                              )[SEARCH_DROP]
        finds_links = driver.find_elements_by_xpath("//a[@class='result__a']"
                                                    )[SEARCH_DROP]

        ############################################################
        info = [line.text for line in finds]
        links = [line.get_attribute('href')
                 for line in finds_links]      # Обработка текста/ссылок
        ############################################################
...
#######################################
        ...
        for line in find:
            if line.get_attribute('aria-label') is None:
                pass
            else:
                find_labels.append(line.get_attribute('aria-label'))

        for line in find:
            if line.get_attribute('href') is None:
                pass
            else:
        ...
        full = [f"[{line}]({line2})"
                for line, line2 in zip(index_list, find_links)]
        full = [f"{val}. {line}"
                for val, line in enumerate(full, 1)]
