...
def edit_message_callback(call):
    ...
    keyboard_black = types.InlineKeyboardButton(text="⚫ Чёрное ⚫",
                                                callback_data="black_roll")
    keyboard_red = types.InlineKeyboardButton(text="🔴 Красное 🔴",
                                              callback_data="red_roll")
    keyboard_green = types.InlineKeyboardButton(text="🟢 Зеленое 🟢",
                                                callback_data="green_roll")
    keyboard_roll = types.InlineKeyboardButton(text="Крутить",
                                               callback_data="roll_fast")
    ...
    bot.edit_message_text(
        ...
        text="_На что желаете поставить ставки? Все ставки закроются через_ "
        "*10 секунд*\n\n" + "\n".join(text_info),
        parse_mode="Markdown", reply_markup=keyboard)


def roll_function() -> Union[None, str]:
    """ Функция для определения победителя и расчета число """
    number = random.randint(1, 100)
    winners_enum = []
    winners = []
    try:  # Если выиграли:
        if number == 1:  # Зеленые
            for key, value in zip(text_info_user["names"],
                                  text_info_user["colors"]):
                if value == "🟢":
                    winners.append(key)
                    win_color = "зеро"
...
def roll(m):
    ...
    # Кнопки, '&&' используется для разделителя,
    # через callback'и отправляем по ним id следующего сообщения
    ...
    bot.send_message(
        m.chat.id,
        "_На что желаете поставить ставки? Все ставки закроются через"
        "_ *10 секунд*",
        parse_mode="Markdown", reply_markup=keyboard)
